...
class salaire_imposable(Variable):
    ...
    unit = EUR
    documentation = (
        "Salaire net imposable d'une personne: salaire brut diminué de la "
        "déduction forfaitaire de 10% pour frais professionnels, bornée par un "
        "plancher et un plafond annuels et limitée au montant du salaire."
    )
    ...
    def formula(individu, period, parameters):
        ...
        abattement = min_(
            salaire_brut,
            min_(p.plafond, max_(p.taux * salaire_brut, p.plancher)),
        )
        abattement = where(salaire_brut > 0, abattement, 0)
        return salaire_brut - abattement
